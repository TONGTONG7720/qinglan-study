...
try {
  ...
  const expectedTables = ["TextbookAsset", "QuestionBankItem", "QuestionBankItemKnowledgeNode", "QuestionBankValidation", "QuestionBankReview", "QuestionBankSemanticDuplicate", "QuestionBankLicenseReview", "QuestionBankRelease"];
  const tables = await client.query(`SELECT COUNT(*)::int AS count FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ANY($1::text[])`, [expectedTables]);
  ...
  if (migrations.rows[0].count < 13 || tables.rows[0].count !== expectedTables.length || hnsw.rows[0].count !== 1 || reviewed.rows[0].invalid !== 0) {
